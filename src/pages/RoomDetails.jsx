...
const RoomDetails = () => {
  ...
  if (isLoading) {
    return (
      <div className="h-screen grid place-items-center text-3xl">Loading...</div>
    );
  }

  if (error) {
    return (
      <div className="h-screen grid place-items-center text-3xl">Room not found</div>
    );
  }

  const { name, description, imageLg, facilities, price } = room;
          ...
          <div className='w-full lg:w-[60%] h-full text-justify'>
            ...
            <div className='mt-12'>
              ...
              <div className="grid grid-cols-3 gap-6 mb-12">
                {
                  facilities.map((item) =>
                    <div key={item.facilityId} className='flex items-center gap-x-3 flex-1'>
          ...
          <div className='w-full lg:w-[40%] h-full'>
            ...
            <div>
              ...
              <ul className='flex flex-col gap-y-4'>
                {
                  hotelRules.map(({ ruleID, rules }) =>
                    <li key={ruleID} className='flex items-center gap-x-4'>
